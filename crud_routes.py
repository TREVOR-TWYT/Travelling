...
@crud.route("/clients/new", methods=["GET", "POST"])
...
def trajets_create():
    if request.method == "POST":
        ...
        db.session.add(t)
        db.session.commit()
        return redirect(url_for("crud.trajets_list"))
...
def create_vehicule():
    if request.method == "POST":
        ...
        vehicule = Vehicule(immatriculation=immatriculation, type=type_, capacite=capacite, statut=statut)
...
def reservation():
    ...
    return render_template("reservation/success.html", reservation=reservation, client=client, paiement=paiement)
